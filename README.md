# 👋 Hi, I'm Adithya S

🚀 Dotnet Developer | MCA Graduate | Transitioning into MERN Stack Development

## 💻 Tech Stack
- **Frontend:** HTML5, CSS3, JavaScript (ES6+), React, Vue.js, Angular
- **Backend:** Node.js, Express.js, ASP.Net (Webforms), C#
- **Database:** MongoDB, SQL Server, MySQL
- **Tools:** Git, GitHub, Webpack, Babel, Jest

## 🌟 Interests
- Building high-performance web applications
- User experience & responsive design
- Continuous learning & experimenting with new frameworks


## 📊 GitHub Stats
![Adithya's GitHub stats](https://github-readme-stats.vercel.app/api?username=adithyasopanam&show_icons=true&theme=radical)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=adithyasopanam&layout=compact&theme=radical)

## 📫 Connect with Me
- LinkedIn: https://www.linkedin.com/in/adithya-s-06april2000/
- Email: adithyas9563@gmail.com
